...
func (m *MessageQuery) collectField(ctx context.Context, oneNode bool, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "messageType":
			if _, ok := fieldSeen[message.FieldMessageType]; !ok {
				selectedFields = append(selectedFields, message.FieldMessageType)
				fieldSeen[message.FieldMessageType] = struct{}{}
			}
		case "id":
		case "__typename":
		default:
...
func (t *ThreadQuery) collectField(ctx context.Context, oneNode bool, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "provider":
			if _, ok := fieldSeen[thread.FieldProvider]; !ok {
				selectedFields = append(selectedFields, thread.FieldProvider)
				fieldSeen[thread.FieldProvider] = struct{}{}
			}
		case "id":
		case "__typename":
		default:
