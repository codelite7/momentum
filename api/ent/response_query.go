...
type ResponseQuery struct {
	...
	loadTotal          []func(context.Context, []*Response) error
...
func (rq *ResponseQuery) QuerySentBy() *AgentQuery {
...
func (rq *ResponseQuery) QueryBookmarks() *BookmarkQuery {
	query := (&BookmarkClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *ResponseQuery) WithSentBy(opts ...func(*AgentQuery)) *ResponseQuery {
	query := (&AgentClient{config: rq.config}).Query()
...
func (rq *ResponseQuery) WithBookmarks(opts ...func(*BookmarkQuery)) *ResponseQuery {
	query := (&BookmarkClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withBookmarks = query
	return rq
}
...
func (rq *ResponseQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Response, error) {
	...
	if rq.withSentBy != nil || rq.withMessage != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := rq.withSentBy; query != nil {
		if err := rq.loadSentBy(ctx, query, nodes, nil,
			func(n *Response, e *Agent) { n.Edges.SentBy = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := rq.withBookmarks; query != nil {
		if err := rq.loadBookmarks(ctx, query, nodes,
			func(n *Response) { n.Edges.Bookmarks = []*Bookmark{} },
			func(n *Response, e *Bookmark) { n.Edges.Bookmarks = append(n.Edges.Bookmarks, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range rq.withNamedBookmarks {
		if err := rq.loadBookmarks(ctx, query, nodes,
			func(n *Response) { n.appendNamedBookmarks(name) },
			func(n *Response, e *Bookmark) { n.appendNamedBookmarks(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (rq *ResponseQuery) loadSentBy(ctx context.Context, query *AgentQuery, nodes []*Response, init func(*Response), assign func(*Response, *Agent)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "agent_responses" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (rq *ResponseQuery) loadMessage(ctx context.Context, query *MessageQuery, nodes []*Response, init func(*Response), assign func(*Response, *Message)) error {
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "message_response" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (rq *ResponseQuery) loadBookmarks(ctx context.Context, query *BookmarkQuery, nodes []*Response, init func(*Response), assign func(*Response, *Bookmark)) error {
	...
	query.Where(predicate.Bookmark(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(response.BookmarksColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.response_bookmarks
		if fk == nil {
			return fmt.Errorf(`foreign-key "response_bookmarks" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "response_bookmarks" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (rq *ResponseQuery) WithNamedBookmarks(name string, opts ...func(*BookmarkQuery)) *ResponseQuery {
	query := (&BookmarkClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if rq.withNamedBookmarks == nil {
		rq.withNamedBookmarks = make(map[string]*BookmarkQuery)
	}
	rq.withNamedBookmarks[name] = query
	return rq
}
